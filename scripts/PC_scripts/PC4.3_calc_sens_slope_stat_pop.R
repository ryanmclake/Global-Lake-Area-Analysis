...
k <- d %>% 
  dplyr::select(hylak_id, pop_sum) %>%
  # Grouping by hylak_id
  dplyr::group_by(hylak_id) %>%
  # Z-score lake area
  mutate(pop_sum = scale(pop_sum)) %>%
  # Summarizing and calculating the sens slope from the trend package
  summarise(across(c(1),  ~list(sens.slope(ts(.))))) %>%
  dplyr::group_by(hylak_id) %>%
  mutate(sens.slope = unlist(purrr::map(pop_sum, "estimates")),
         p.value = unlist(purrr::map(pop_sum, "p.value"))) %>%
  select(-pop_sum) %>%
...
left_join(d, k, by = "hylak_id") %>%
  ...
  write_feather(., path = paste0("./outputs/PC5.3_add_sens_slope_stat_pop.feather"))
