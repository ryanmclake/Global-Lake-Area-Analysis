...
k <- d %>% 
  dplyr::select(hylak_id, total_precip_mm) %>%
  # Grouping by hylak_id
  dplyr::group_by(hylak_id) %>%
  # Z-score lake area
  mutate(total_precip_mm = scale(total_precip_mm)) %>%
  # Summarizing and calculating the sens slope from the trend package
  summarise(across(c(1),  ~list(sens.slope(ts(.))))) %>%
  dplyr::group_by(hylak_id) %>%
  mutate(sens.slope = unlist(purrr::map(total_precip_mm, "estimates")),
         p.value = unlist(purrr::map(total_precip_mm, "p.value"))) %>%
...
left_join(d, k, by = "hylak_id") %>%
  ...
  write_feather(., path = paste0("./outputs/PC5.1_add_sens_slope_stat_precip.feather"))
